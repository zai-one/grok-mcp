...
Options:
  ...
  --lanes PATH       Worktree lanes parent (default: <project>/../.grok-mcp-lanes)
...
if [ -z "$LANES_PARENT" ]; then
  LANES_PARENT="$(dirname "$PROJECT_ROOT")/.grok-mcp-lanes"
fi
mkdir -p "$LANES_PARENT"
LANES_PARENT="$(cd "$LANES_PARENT" && pwd -P)"
