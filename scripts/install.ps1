...
# Windows PowerShell 5.1 writes a BOM for -Encoding UTF8, and a strict JSON
# parser refuses a document that starts with one -- Node's JSON.parse does, and
# so does Python's json.loads. The snippet then looks like a host bug rather
# than an installer bug. WriteAllText with a no-BOM encoder is the same call on
# 5.1 and 7.
...
# Lanes default to <project>/.grok/lanes inside the bridge. Pinning a sibling
# of the project here put unmerged work where README and AGENTS.md say it
# must not go, and pytest would walk it.
...
Write-Utf8NoBom (Join-Path $mcpDir "claude_desktop.snippet.json") $claudeSnippet
...
Write-Utf8NoBom (Join-Path $mcpDir "cursor.snippet.json") $cursorSnippet
