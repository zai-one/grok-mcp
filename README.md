...
закончишь» инвертировала отказ — 50 ходов вхолостую и `VERDICT_MISSING`. Эскалирующая подсказка
`[driver retry 2]` из `_WRITE_FIRST_NUDGES` сделала хуже: вердикт вышел на первом ходу.

Та же цель с `lane_verdict: false` — четыре хода и реальный дифф на два файла. Дефолт остаётся
включённым: разобранный вердикт полезен, когда цель достаточно мала, чтобы до него дойти.
