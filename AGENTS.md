...
- HTTP-транспорт требует `GROK_DELEGATE_HTTP_TOKEN` **везде**, включая loopback: loopback делят все процессы машины и браузер, а за этим эндпоинтом job-инструменты. Без токена сервер не поднимется; открыт только `/healthz`.
